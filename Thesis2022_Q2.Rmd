...
# Back transform forbs (Assessing 9/22/22)

```{r}
backFB <- emmeans(LMFitsqrtFB, pairwise ~ factor(forest_type), transform = "response")
backFB
...
LogisFitSH <- glm(BinaryCover ~ burn_sev_category, data=ShrubCoverBinary, family= "binomial" )

library(car)
library(emmeans)
summary(LogisFitSH)
Anova(LogisFitSH,type=3)
emmeans(LogisFitSH,pairwise~burn_sev_category,type="response")
```
...
LogisFitTR <- glm(BinaryCover ~ burn_sev_category, data=TreeCoverBinary, family= "binomial" )

summary(LogisFitTR)
Anova(LogisFitTR, type=3)
emmeans(LogisFitTR,pairwise~burn_sev_category, type="response")
```
