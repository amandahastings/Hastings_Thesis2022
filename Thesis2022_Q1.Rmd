...
QUGAdensity <- ThesisAll1 %>% 
  dplyr::select(PlotID, burn_sev_category, forest_type, QUGA_total)%>% 
  filter(forest_type == "Lower Montane")


hist(sqrt(QUGAdensity$QUGA_total))

QUGALMFit <- lm(sqrt(QUGA_total) ~ burn_sev_category, data = QUGAdensity)
anova(QUGALMFit)
par(mfrow = c(1, 2))
...
RONEdensity <- ThesisAll1 %>% 
  dplyr::select(PlotID, burn_sev_category, forest_type, RONE_total)%>% 
  filter(forest_type == "Lower Montane")
