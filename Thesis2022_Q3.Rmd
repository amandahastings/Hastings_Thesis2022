...
z_NMDSdf_LowerMontane <- ThesisNMDSResponse %>% 
  ...
  column_to_rownames(var="PlotID")%>% 
   rename(
      ...
      "PICO" = z_PICO_total,
      "PIFL" = z_PIFL_total)%>% 
  dplyr::select(!c(gm_cover, fb_cover, sh_cover, tr_cover, POTR_total, QUGA_total, RONE_total, PIPO_total, PSME_total, ABCO_total, PICO_total, PIFL_total))
...
z_OverlayPred_LowerMontane <- OverlayPredictors1 %>% 
  filter(forest_type == "Lower Montane")%>% 
...
#Overlay Predictors Lower Montane 
z_OverlayPred_UpperMontane <- OverlayPredictors1 %>% 
  filter(forest_type == "Upper Montane")%>% 
  dplyr::select(!forest_type) %>% 
  mutate(across(canopy_cover:organic_cover,~as.numeric(scale(.)), .names = 'z_{col}')) %>%
  replace(is.na(.),0)%>%
...
#Overlay Predictors Lower Montane 
z_OverlayPred_Subalpine <- OverlayPredictors1 %>% 
  filter(forest_type == "Subalpine")%>% 
  dplyr::select(!forest_type) %>% 
...
# text(NMDS_LowerMontane, display = "spec", cex=0.7, col="blue")
