...
AnnualDeviation <- inner_join(AnnualAvgTerra,PostAnnualJoin)%>% 
...
GrowDeviation <- inner_join(GrowSeasonAvgTerra,PostGrowTerraJoin)%>%
  ...
  mutate("dev2021Precip" = (mean3yrPrecip - meanPrecip)/sdPrecip)%>%
  mutate("dev2021Soil"= (mean3yrSoil - meanSoil)/sdSoil)
...
```{r}
ThesisAll <- cbind(ThesisData, ThesisTerra)
```

## Full data frame for each forest type 

```{r}
LowerMontaneAll <- ThesisAll %>% 
  filter(forest_type == "Lower Montane")

UpperMontaneAll <- ThesisAll %>% 
  filter(forest_type == "Upper Montane")

SubalpineAll <- ThesisAll %>% 
