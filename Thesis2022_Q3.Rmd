...
z_NMDSdf_LowerMontane <- ThesisNMDSResponse %>% 
  ...
  dplyr::select(!c(z_PIPO_total,z_PIFL_total, z_tr_cover))%>% 
  rename(
    "Graminoids" = z_gm_cover,
    "Forbs" = z_fb_cover,
    "Shrubs" = z_sh_cover,
...
#NMDS Upper Montane 
NMDS_UpperMontane <- metaMDS(max_NMDSdf_UpperMontane, distance = "euclidean", k=2, autotransform = F)
ordiplot(NMDS_UpperMontane, type = 't', main = "Upper Montane")
str(NMDS_UpperMontane)
vegan:: stressplot(NMDS_UpperMontane)

#NMDS Subalpine 
NMDS_Subalpine <- metaMDS(max_NMDSdf_Subalpine, distance = "euclidean", k=2, autotransform = F)
