...
#Lower Montane NMDS
z_NMDSdf_LowerMontane <- z_NMDSResponse %>% 
  filter(forest_type == "Lower Montane")%>% 
  dplyr::select(!c(forest_type, z_PIAR_total, z_ABLA_total, z_PIEN_total))


#Upper Montane NMDS  
z_NMDSdf_UpperMontane <- z_NMDSResponse %>% 
  filter(forest_type == "Upper Montane")%>% 
  dplyr::select(!c(forest_type))

#Subalpine NMDS
z_NMDSdf_Subalpine <- z_NMDSResponse %>% 
  filter(forest_type == "Subalpine")%>% 
  dplyr::select(!c(forest_type, z_QUGA_total, z_RONE_total, z_PIPO_total))
...
#NMDS Lower Montane 
NMDS_LowerMontane <- metaMDS(z_NMDSdf_LowerMontane, distance = "euclidean", k=2, autotransform = F)
