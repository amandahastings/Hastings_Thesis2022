...
fort.plot_Families <- merge(fort_Families, families_NMDS_pred, by.x = "Label", by.y = "PlotID", all = T)
...
```{r}
adonis2(families.NMDS.nopred ~burn_sev_category, data = families_NMDS_pred)
```
...
```{r}
#Families NMDS plot
FamiliesNMDSplot <- ggplot() +
    geom_point(
    data = subset(fort.plot_Families, Score == "sites"),size = 1,
    alpha = 0.75,
    shape = "circle",
    stroke = 1,
  ...
  labs(title = "Plant families across sites") +
  geom_text(
